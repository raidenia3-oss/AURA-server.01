...
try:
    ...
    OSINTEngine = None
    print("[WARN]  OSINTEngine no disponible (osint_engine.py no encontrado)")
...
try:
    ...
    print("[CHECK] Skills Forge cargado correctamente")
except ImportError:
    execute_skill_chain = None
    list_available_tools = None
    print("[WARN]  Skills Forge no disponible (skills_forge.py no encontrado en AURA_Core/)")
except Exception as e:
    execute_skill_chain = None
    list_available_tools = None
    print(f"[WARN]  Error cargando Skills Forge: {e}")
...
try:
    ...
    print("[CHECK] Telemetría radio cargada correctamente")
except ImportError:
    generate_wifi_radar_data = None
    start_wifi_watchdog = None
    stop_wifi_watchdog = None
    get_wifi_watchdog_status = None
    print("[WARN]  Telemetría radio no disponible (telemetria_radio.py no encontrado)")
except Exception as e:
    generate_wifi_radar_data = None
    start_wifi_watchdog = None
    stop_wifi_watchdog = None
    get_wifi_watchdog_status = None
    print(f"[WARN]  Error cargando telemetria_radio: {e}")

# ──── Lógica de Decisión Inteligente ────
class TaskDecisionEngine:
    """Motor de decisión inteligente para delegación de tareas"""

    def __init__(self):
        self.hf_endpoint = os.getenv('HF_SPACE_URL', 'https://raiden456-slut.hf.space/v1')
        self.hf_api_key = os.getenv('HUGGINGFACE_API_KEY')
        self.local_threshold = 4 * 1024 * 1024 * 1024  # 4GB en bytes
        self.fast_task_types = {'script', 'bot', 'automation', 'webhook', 'api_call'}
        self.heavy_task_types = {'llm', 'model', 'training', 'inference', 'embedding'}

    def analizar_tarea(self, tarea: dict) -> dict:
        """Analiza la complejidad de una tarea"""
        analysis = {
            'tipo': tarea.get('type', 'unknown'),
            'memoria_estimada': tarea.get('estimated_memory', 0),
            'tiempo_estimado': tarea.get('estimated_time', 0),
            'requiere_gpu': tarea.get('requires_gpu', False),
            'modelo': tarea.get('model', None),
            'es_grande': False,
            'es_rapida': False
        }

        # Determinar si es una tarea grande
        if (analysis['memoria_estimada'] > self.local_threshold or
            analysis['requiere_gpu'] or
            analysis['modelo'] or
            analysis['tipo'] in self.heavy_task_types):
            analysis['es_grande'] = True

        # Determinar si es una tarea rápida
        if (analysis['tipo'] in self.fast_task_types or
            analysis['tiempo_estimado'] < 30):  # Menos de 30 segundos
            analysis['es_rapida'] = True

        return analysis

    def decidir_procesamiento(self, tarea: dict) -> dict:
        """
        Decide dónde procesar una tarea:
        - Tareas grandes (modelos, >4GB RAM, GPU) → Hugging Face
        - Tareas rápidas (scripts, bots, automatización) → Local
        """
        decision = {
            'tarea_id': tarea.get('id', 'unknown'),
            'location': 'local',
            'reason': 'Tarea rápida o de baja complejidad',
            'endpoint': None,
            'analysis': self.analizar_tarea(tarea)
        }

        # Delegar a Hugging Face si es una tarea grande
        if decision['analysis']['es_grande']:
            decision['location'] = 'huggingface'
            decision['reason'] = 'Tarea compleja que requiere recursos significativos'
            decision['endpoint'] = self.hf_endpoint

        return decision

    def ejecutar_tarea(self, tarea: dict) -> dict:
        """Ejecuta una tarea según la decisión tomada"""
        decision = self.decidir_procesamiento(tarea)
        resultado = {
        ...
        try:
            if decision['location'] == 'local':
                resultado['result'] = self._ejecutar_local(tarea)
                resultado['status'] = 'completed'
            else:
                resultado['result'] = self._ejecutar_huggingface(tarea, decision['endpoint'])
                resultado['status'] = 'completed'
    ...
    def _ejecutar_local(self, tarea: dict) -> dict:
        """Ejecuta una tarea localmente"""
        # Implementación simplificada - en producción usaría subprocess o threading
        return {
            'execution': 'local',
            'start_time': time.time(),
            'end_time': time.time() + 5,  # Simular ejecución
            'resource_usage': {
                'cpu': 25,  # %
                'memory': 512  # MB
            }
    ...
    def _ejecutar_huggingface(self, tarea: dict, endpoint: str) -> dict:
        """Ejecuta una tarea en Hugging Face"""
        import requests
        import json
...
decision_engine = TaskDecisionEngine()
...
try:
    ...
    print("[CHECK] Tunnel manager cargado correctamente")
except Exception as e:
    tunnel_mgr = None
    print(f"[WARN]  Tunnel manager no disponible: {e}")

try:
    if start_wifi_watchdog is not None:
        start_wifi_watchdog()
        print("[CHECK] WiFi watchdog iniciado")
except Exception as e:
    print(f"[WARN]  No se pudo iniciar WiFi watchdog: {e}")
        ...
        return jsonify(
            {
                "status": "ok",
                "briefing": None,
                "message": "No hay briefing aún. Usa POST /api/briefing/refresh",
            }
        )
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
...
def api_tools_execute():
    ...
    try:
        ...
        if not tool_info:
            return (
                jsonify({"status": "error", "message": f"Herramienta '{tool_name}' no registrada"}),
                404,
            )

        result = execute_single_tool(tool_name, params=params, run_in_background=run_in_background)
...
def api_decision_analyze():
    """
    Analiza una tarea y decide dónde procesarla.
    Body JSON: { "task": { "type": "llm", "estimated_memory": 6000000000, ... } }
    """
    data = request.get_json(force=True)
    if not data or "task" not in data:
        return jsonify({"status": "error", "message": "task requerido"}), 400

    try:
        analysis = decision_engine.analizar_tarea(data["task"])
        return jsonify({
            "status": "ok",
            "analysis": analysis,
            "recommendation": "huggingface" if analysis["es_grande"] else "local"
        })
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500

@app.route("/api/decision/execute", methods=["POST"])
def api_decision_execute():
    """
    Ejecuta una tarea según la decisión inteligente.
    Body JSON: { "task": { "id": "task1", "type": "llm", ... } }
    """
    data = request.get_json(force=True)
    if not data or "task" not in data:
        return jsonify({"status": "error", "message": "task requerido"}), 400
    ...
    try:
        result = decision_engine.ejecutar_tarea(data["task"])
        return jsonify(result)
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500

@app.route("/api/decision/decide", methods=["POST"])
def api_decision_decide():
    """
    Solo decide dónde procesar sin ejecutar.
    Body JSON: { "task": { "type": "llm", "estimated_memory": 6000000000, ... } }
    """
    data = request.get_json(force=True)
    if not data or "task" not in data:
    ...
    try:
        decision = decision_engine.decidir_procesamiento(data["task"])
        return jsonify({
            "status": "ok",
            "decision": decision
        })
...
def api_osint():
    ...
    if OSINTEngine is None or osint_engine is None:
        return (
            jsonify({"error": "OSINTEngine no disponible. Verifica osint_engine.py en AURA_Core/"}),
            500,
        )
...
def api_import_watchlist():
    ...
    return jsonify(
        {"status": "ok", "imported_count": len(imported), "total_watchlist": len(WATCHLIST_STORE)}
    )
...
def api_system_verify():
    ...
    """
    verification = {"timestamp": time.strftime("%Y-%m-%dT%H:%M:%S"), "checks": {}}
    ...
    components = {"ai_router": False, "osint_engine": False, "skills_forge": False}
    ...
    verification["overall_status"] = (
        "healthy" if all_ok else "warning" if (watchlist_ok and alerts_ok) else "critical"
    )
    ...
    stats = {
        ...
        "watchlist": {
            ...
            "by_priority": {
                ...
                "medium": len(
                    [t for t in WATCHLIST_STORE.values() if t.get("priority") == "medium"]
                ),
...
def start_service(service):
    ...
    try:
        ...
        with open(log_path, "a") as log_file:
            srv["process"] = subprocess.Popen(cmd, stdout=log_file, stderr=log_file, cwd=script_dir)
...
def api_health_log():
    ...
    health_log_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "AURA_Core", "system_health.log"
    )
    try:
...
def api_evolution_proposals():
    ...
    try:
        ...
        files = ["AURA_Core/ai_router.py", "AME_Core/servidor_ame.py", "AURA_Core/osint_radar.py"]
    ...
    if data["perturbation"] > 40:
        try:
            _inject_ticker_alert(
                "warning", f"[RADAR] ALTA PERTURBACIÓN CSI: {data['perturbation']}%", "sensor"
            )
        except:
            pass

    return jsonify(
        {"status": "standby", "received": True, "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S")}
    )
...
def api_sensor_acoustic():
    ...
    if data["amplitude"] > 1.2:
        try:
            _inject_ticker_alert(
                "critical",
                f"[SOUND] ECO ACÚSTICO ANÓMALO: {data['amplitude']} @ {data['frequency']}Hz",
                "sensor",
            )
        except:
            pass
...
def skill_forge_execute():
    ...
    if not skills and edges:
        ...
        for e in edges:
            ...
            if s in tool_map_ids and s != "master":
                connected.add(s)
            if t in tool_map_ids and t != "master":
                connected.add(t)
    ...
    print(f"[HAMMER] [Skill Forge] Flow: {flow} | Skills: {skills} | Target: {target}")
                ...
                try:
                    result = execute_skill_chain(
                        ...
                        output_dir=os.path.join(
                            os.path.dirname(os.path.abspath(__file__)), "forge_output"
                        ),
                    )
    ...
    tool_names = {
        "osint": "[MAG] OSINT Engine",
        "blackbird": "🐦 Blackbird/Nexfil",
        "photon": "🌐 Photon Crawler",
        "exiftool": "[CAMERA] ExifTool",
        "termux": "[PHONE] Termux Sync",
        "ollama": "[BRAIN] Ollama LLM",
    }
    results = {}
    for skill in skills:
        name = tool_names.get(skill, skill)
        results[skill] = f"[CHECK] {name} ejecutado (simulado — skills_forge.py no disponible)"

    if combined:
        flow_label = combined.get("label", "custom")
        results["pipeline"] = f"[REFRESH] Flujo combinado: {flow_label} (simulado)"
...
def api_radar():
    ...
    try:
        ...
        return jsonify(
            {
                "status": "ok",
                "summary": "[RADAR] Radar sin datos. Verifica conexión a internet y feeds RSS.",
                "tactical_html": "<p>[RADAR] Radar sin datos. Verifica conexión a internet y feeds RSS.</p>",
                "headlines": [],
...
def update_mobile_ip():
    ...
    ip_file = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "AURA_Core", "config.json"
    )
    try:
        ...
        print(f"[PHONE] [AME Server] IP móvil actualizada vía webhook: {new_ip}")
    ...
    try:
        ...
        if wifi_data.get("presence_detected") and wifi_data.get("perturbation_index", 0) > 40:
            try:
                _inject_ticker_alert(
                    "warning",
                    f"[RADAR] PRESENCIA DETECTADA — CSI Perturbation: {wifi_data['perturbation_index']}% | SNR: {wifi_data['snr_avg']}dB",
                    "wifi_radar",
                )
            except Exception:
                pass
...
def api_wifi_spectrum():
    ...
    for ch in range(1, 14):
        ...
        spectrum_analysis[f"ch_{ch}"] = {
            ...
            "interference_risk": (
                "high" if occupancy > 70 else "medium" if occupancy > 40 else "low"
            ),
            "ap_count": _rnd.randint(0, 8) if occupancy > 30 else 0,
        }

    # Recomendar mejor canal (menos interferencia)
    best_channel = min(spectrum_analysis.items(), key=lambda x: x[1]["occupancy_percent"])

    # Canales problemáticos
    congested_channels = [
        ch for ch, data in spectrum_analysis.items() if data["interference_risk"] == "high"
    ]
    ...
    return jsonify(
        {
            ...
            "overall_health": (
                "good"
                if len(congested_channels) <= 2
                else "fair" if len(congested_channels) <= 5 else "poor"
            ),
        }
    )
...
def _save_watchlist(watchlist):
    ...
    try:
        ...
        print(f"[WARN]  Error guardando watchlist: {e}")
...
def api_watchlist_add():
    ...
    try:
        _inject_ticker_alert(
            "warning", f"🎯 Nuevo objetivo añadido: {data['target']} [{data.get('type', 'domain')}]"
        )
    except:
        pass
...
def _default_alerts():
    """Alertas por defecto."""
    return [
        {"type": "info", "message": "[GREEN] Sistema AURA operativo", "source": "system"},
        {"type": "info", "message": "[YELLOW] Monitor de amenazas activo", "source": "system"},
        {"type": "info", "message": "[BLUE] VOID memory sincronizada", "source": "system"},
    ]
...
def _save_ticker(alerts):
    ...
    try:
        ...
        print(f"[WARN]  Error guardando ticker: {e}")
...
def api_get_latest_intel():
    ...
    for alert in TICKER_MESSAGES[-12:]:
        icon = "⎔" if alert["type"] == "info" else "[WARN]" if alert["type"] == "critical" else "◈"
        formatted_items.append(f"{icon} {alert['message']}")
    return jsonify(
        {
            "status": "ok",
            "items": (
                formatted_items if formatted_items else ["[RADAR] AURA OPERATIVO · Sistema en línea"]
            ),
        }
    )
...
def api_ticker_push():
    ...
    return jsonify(
        {
            "status": "ok",
            "alert_injected": {"type": alert_type, "message": message, "source": source},
        }
    )
...
def api_osint_integrate():
    ...
    for alert in alerts:
        ...
        message = f"[RADAR] {title}: {alert.get('description', '')[:80]}"
...
def api_situation_report():
    ...
    try:
        ...
        for t in active_targets:
            ...
            report["watchlist_summary"]["targets_by_type"][ttype] = (
                report["watchlist_summary"]["targets_by_type"].get(ttype, 0) + 1
            )
    except Exception as e:
...
if __name__ == "__main__":
    print("[ROCKET] AURA Command Center — Servidor Flask v3 + WebSocket")
    print(f"[RADAR] Escuchando en: http://0.0.0.0:5000")
    print(f"[BAR_CHART] Dashboard:    http://localhost:5000/")
    print(f"[WRENCH] API Status:   http://localhost:5000/api/status")
    print(f"[PHONE] OTA APK:      http://0.0.0.0:5000/api/descargar-ame")
    print(f"[PLUG] WebSocket:    ws://0.0.0.0:5000 (CORS: *)")
    print("=" * 50 + "\n")
