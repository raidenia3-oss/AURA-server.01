...
const AURA_URL = "https://aura-server-01.vercel.app";
...
async function checkForUpdates() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Error al verificar versión: ${response.status} ${response.statusText}`,
            );
        }
...
async function triggerUpdateProcess(serverVersion) {
    ...
    try {
        ...
        try {
            ...
            updateModal.querySelector(".update-message").textContent =
...
function showUpdateModal(serverVersion) {
    ...
    updateModal.querySelector(".update-message").textContent =
        `🔄 Actualización disponible: v${serverVersion.version}`;
}
