...
class WebSocketServer:
    ...
    async def _simulate_task_execution(self, task_id: str, node_id: str, module_type: str):
        ...
        if task_id in self.tasks:
            ...
            task['progress'] = 100
            task['output'].append(f"[{datetime.now().strftime('%H:%M:%S')}] Task completed successfully on node {node_id}")
            await self._broadcast('task_update', task)
    ...
    async def _broadcast(self, event: str, data: Dict):
        ...
        if event in self.subscriptions:
            for connection_id in self.subscriptions[event]:
                if connection_id in self.websockets:
                    try:
                        message = {
                            "event": event,
                        ...
                        await self._send_to_connection(connection_id, event, data)
                    except Exception as e:
